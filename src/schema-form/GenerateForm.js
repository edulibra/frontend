 ...
 * @returns {*|{}}
 */
const addValidator = (item, componentProps) => {
  const decorator = item.decoratorOption || {};
  ...
  decorator.rules.push({
    validator: (rule, value, callback) => {
      item.validator(rule, value, callback, componentProps); //, this.props
    },
  });
  return decorator;
};
...
class GenerateForm extends React.Component {
  ...
  getFields = () => {
    ...
    const formData = this.props.formData || this.props.documentData;
    ...
    elements.map((item, i) => {
      ...
      let decorator = addValidator(item, this.props);
