...
export const executeAutoFetch = async (config) => {
  ...
  const attachedNodes = params.attachedNodes ? params.attachedNodes : {};
  const node = config.node || params.node;
  const fetchAction = config.fetchAction || 'find';
  const url = config.url || Endpoints.node.detail;
  ...
  routes.map((r) => {
    result.push({
      ...
      component: ((props) => {
        ...
        const { documentIid, routeParams } = getDocumentIid(props);

        if (!r.autoFetch || (!documentIid && routeParams.length === 0)) {
          return <Component {...props} routeParams={{ ...routeParams }} route={r} popupScreenId={popupScreenId} />;
        }
        ...
        return <Component {...props} {...autoSetValues} route={r} routeParams={{ ...routeParams }} iid={documentIid} popupScreenId={popupScreenId} />;
      }),
    });
  });
